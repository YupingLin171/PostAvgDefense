...
sample_method = ['random'] # options = ['random', 'approx']
shift_direction = 'both' # options = ['both', 'inc', 'dec']
vote_method = 'avg_softmax' # options = ['avg_feat', 'most_vote', 'weighted_feat', 'avg_softmax']

# define paths
dataDir = '/local/ssd/yuping/videoLearning/ILSVRC2012/val'
showCharts = True
showNoiseMaps = False
showCorrectSamples = False
chartSaveDir = './rresults_FGSM_N5000/expm_random_sfmx_K15_r10-20-30_linf8_255'
...
dataset = datasets.ImageFolder(dataDir, transform=transforms.Compose([transforms.Resize(256), transforms.CenterCrop(224), transforms.ToTensor()]))
if kImgs == 'all':
    kImgs = len(dataset)

# setup pretrained model
model = mdl.resnet152(pretrained=True)
